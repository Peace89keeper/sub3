...
const Login = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // THE REAL AXIOS COMMAND (Uncomment when Express backend is ready)
      // const response = await axios.post('http://localhost:5000/api/auth/login', credentials);
      // const token = response.data.token;

      // Simulating a successful backend response for the frontend build
      console.log("Sending credentials to backend:", credentials);
      const simulatedResponse = {
        data: { token: "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.dummy.token" },
      };
      const token = simulatedResponse.data.token;

      // Store the JWT in localStorage
      localStorage.setItem("schemeMatch_token", token);
      console.log("JWT VIP Wristband secured in localStorage.");
        ...
        <div className="sm-login-wrapper">
          <div className="sm-login-header">
            ...
            <p className="sm-login-subtitle">
              Log in to see your updated Personal Impact Scores and new scheme
              alerts.
            </p>
          </div>
          ...
          <form className="sm-form" onSubmit={handleSubmit}>
            ...
            <div className="sm-input-group">
              ...
              <div className="sm-password-wrapper">
                ...
                <button
                  ...
                  tabIndex="-1" // Prevents the tab key from focusing the icon instead of the next input
                >
                  {showPassword ? (
                    // Eye Slash SVG (Hide)
                    <svg
            ...
            <button
              ...
              disabled={isLoading}
            >
              <Link to="/dashboard" className="sm-link">
          ...
          <div className="sm-login-footer">
            <p>
              ...
              <span className="sm-link-highlight">Build your profile</span>
            </p>
          </div>
        </div>
      </main>
